...
static uintn Mouse_rightButton = 0;
...
void Mouse_CheckState(void) {
    ...
    for(uintn i=0; i<Efi_SimplePointerProtocol_Count; i++) {
        ...
        if(status == 0) {
            ...
            if(Efi_Mouse_State.RightButton) Mouse_rightButton = 1;
        }
    }
    if(updateFlag) {
        ...
        Layer_Mouse_NotifyUpdate(Mouse_x, Mouse_y, Mouse_leftButton, Mouse_rightButton);
    }
